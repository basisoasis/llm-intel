...
import type { ModelData, ModelResult, ModelsResult } from "./types/models";
...
export class LLMIntelClient {
  ...
  constructor(opts: LLMIntelClientOptions) {
    if (Array.isArray(opts.models)) {
      this.validateModelData(opts.models, "constructor");
    }
  ...
  private validateModelData(data: unknown, source: string): ModelData[] {
    try {
      return parse(array(ModelDataSchema), data);
    } catch (err) {
      if (isValiError(err)) {
        throw new Error(
  ...
  private async resolveModels(): Promise<ModelData[]> {
    ...
    if (!res.ok) {
      throw new Error(`[LLMIntelClient] Failed to fetch models from "${url}": HTTP ${res.status}`);
    }

    const json = await res.json();

    // Accept either a raw ModelData[] or a ModelsResult envelope
    const payload = Array.isArray(json) ? json : (json as Record<string, unknown>).data;
   ...
   */
  calculateCost(model: ModelResult, tokens: TokenInput, currency: CostCurrency = "USD"): CostResult {
